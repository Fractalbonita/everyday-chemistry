<template>
  <div class="bubble-container">
    <svg
      xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"
      xmlns="http://www.w3.org/2000/svg"
      inkscape:version="0.48.3.1 r9886"
      viewBox="0 0 507.81 444.37"
      width="275"
      height="275"
      version="1.1"
      inkscape:output_extension="org.inkscape.output.svg.inkscape"
    >
      <defs id="defs550">
        <filter
          id="filter3215"
          inkscape:collect="always"
          color-interpolation-filters="sRGB"
        >
          <feGaussianBlur
            id="feGaussianBlur3217"
            stdDeviation="6.6216798"
            inkscape:collect="always"
          />
        </filter>
      </defs>
      <title id="speech-bubble" lang="en">speech bubble</title>
      <g>
        <path
          id="path710"
          width="250"
          height="250"
          inkscape:connector-curvature="0"
          style="filter:url(#filter3215);fill-rule:evenodd;fill:#000000;fill-opacity:.49804"
          d="m84.128 19.564 336.96 2.398c20.81 0.149 56.06 5.474 63.55 34.775 8.66 38.537 11.55 211.95-2.4 238.63-10.28 21.53-19.82 28.7-29.97 34.78-30.07 14.44-170.28 11.99-170.28 11.99l-21.59 86.34-41.97-87.54h-145.09c-20.362 0-38.981-12.8-45.57-40.77-11.471-50.78-13.704-193.21-2.398-226.64 10.714-30.537 28.721-54.174 58.758-53.96z"
        />
        <path
          id="path559"
          width="250"
          height="250"
          inkscape:connector-curvature="0"
          style="stroke-linejoin:round;fill-rule:evenodd;stroke:#000000;stroke-width:5;fill:#ffffff"
          d="m71.183 5.0014 336.96 2.398c20.82 0.148 56.06 5.474 63.56 34.775 8.66 38.53 11.55 211.95-2.4 238.63-10.28 21.52-19.83 28.7-29.98 34.78-30.06 14.44-170.28 11.99-170.28 11.99l-21.58 86.33-41.97-87.53h-145.1c-20.357 0-38.976-12.81-45.564-40.77-11.469-50.79-13.702-193.22-2.396-226.64 10.714-30.536 28.721-54.173 58.758-53.959z"
        />
      </g>
    </svg>
    <div class="bubble-wrapper">
      <slot></slot>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent } from 'vue';

export default defineComponent({
  name: 'SpeechBubble'
});
</script>

<style lang="scss" scoped>
.bubble {
  &-container {
    position: relative;
  }
  &-wrapper {
    position: absolute;
    z-index: 1;
    top: 30px;
    left: 20px;
    width: 225px;
    height: 150px;
    display: flex;
    align-items: center;
    justify-content: center;
  }
}
</style>
